...
export const api = {
  ...
  uploadApk: async (appId, file, version, releaseNotes, buildNumber) => {
    const token = localStorage.getItem('admin_token');
    const formData = new FormData();
    formData.append('apk', file);
    if (version) formData.append('version', version);
    if (releaseNotes) formData.append('releaseNotes', releaseNotes);
    if (buildNumber) formData.append('buildNumber', String(buildNumber));
  ...
  updateDeviceDatabase: (licenseId, deviceId, body) =>
    request(`/admin/licenses/${licenseId}/devices/${deviceId}/database`, { method: 'PUT', body }),
